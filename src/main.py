...
def main():
    ...
    criterion_t = torch.nn.KLDivLoss(reduction="none", log_target=True)
    ...
    if param['ablation_mode'] == 0:
        ...
        _, test_acc, test_val, test_best, _ = train_student(param, model_s, g, feats, labels, out_t, indices, criterion_l, criterion_t, evaluator, optimizer_s) 
        ...
        model_s = Model(param, model_type='student').to(device)
        optimizer_s = optim.Adam(model_s.parameters(), lr=float(param["learning_rate"]), weight_decay=float(param["weight_decay"]))
        out_s, test_acc, test_val, test_best, state_s = train_student(param, model_s, g, feats, labels, out_t, indices, criterion_l, criterion_t, evaluator, optimizer_s)  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dropout_s", type=float, default=0.3)
